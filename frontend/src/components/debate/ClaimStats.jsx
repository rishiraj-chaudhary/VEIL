  ...
  if (compact && !expanded) {
    return (
      <button
        ...
        className="inline-flex items-center space-x-2 px-3 py-1 bg-slate-900/20 border border-slate-700/50 rounded-full text-xs text-slate-400 hover:bg-slate-900/30 transition-colors"
      >
        <span>📊</span>
    ...
    <div className="bg-slate-800/50 border border-slate-700/30 rounded-lg p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-slate-900/50 rounded p-3">
          ...
          <div className="text-xs text-slate-400">Times Refuted</div>
        </div>
        ...
        <div className="pt-2 border-t border-slate-700">
          ...
          <div className="space-y-1">
            {stats.relatedClaims.slice(0, 3).map((related, idx) => (
              <div
                key={idx}
                className="text-xs text-slate-300 bg-slate-900/30 rounded p-2"
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-slate-400">{related.similarity}% similar</span>
                  <span className="text-slate-500">{related.uses} uses</span>
                </div>
                <div className="text-slate-400">"{related.text.substring(0, 80)}..."</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="pt-2 border-t border-slate-700">
          ...
          <div className="space-y-1">
            {stats.counterClaims.slice(0, 2).map((counter, idx) => (
              <div
                key={idx}
                className="text-xs text-slate-300 bg-red-900/10 border border-red-700/30 rounded p-2"
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-red-400">
                    {counter.effectiveness}/10 effectiveness
                  </span>
                  <span className="text-slate-500">{counter.uses} uses</span>
                </div>
                <div className="text-slate-400">"{counter.text.substring(0, 80)}..."</div>
              </div>
            ))}
          </div>
        </div>
