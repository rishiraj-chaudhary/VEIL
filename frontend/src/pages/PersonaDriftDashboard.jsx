  ...
  return (
    <div className="mb-4">
      <div className="flex justify-between text-sm mb-1">
        ...
        <span className="text-slate-500 text-xs">You: <span className="text-slate-300">{selfVal}</span>{' · '}Others: <span className={isHigher ? 'text-orange-300' : 'text-green-300'}>{perceivedVal}</span></span>
      </div>
      <div className="relative h-3 bg-slate-700 rounded-full overflow-hidden">
        <div className="absolute h-full bg-slate-500/60 rounded-full" style={{ width: `${selfVal}%` }} />
        <div className="absolute h-full border-2 border-orange-400 rounded-full" style={{ width: `${perceivedVal}%` }} />
      </div>
      <div className="flex justify-between text-xs mt-1 text-slate-600"><span>0</span><span>100</span></div>
    </div>
  );
};

const InsightCard = ({ insight, index }) => (
  <div className="bg-slate-700/50 border border-slate-500/20 rounded-lg p-4 text-sm text-slate-300 leading-relaxed" style={{ animationDelay: `${index * 100}ms` }}>{insight}</div>
);

const TrendBadge = ({ trend }) => {
  const config = {
    improving: { color: 'text-green-400 bg-green-900/30 border-green-500/30', icon: '↑', label: 'Improving' },
    stable:    { color: 'text-slate-400 bg-slate-900/30 border-slate-500/30',   icon: '→', label: 'Stable' },
  ...
  }[trend] || { color: 'text-slate-400 bg-slate-700 border-slate-600', icon: '?', label: trend };
...
const PersonaDriftDashboard = () => {
  ...
  if (loading && !latestSnapshot) return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-900 to-slate-900 flex items-center justify-center">
      <div className="text-center"><div className="animate-spin rounded-full h-16 w-16 border-b-2 border-slate-500 mx-auto mb-4" /><div className="text-white text-xl">Loading your persona data...</div></div>
    </div>
  );
  ...
  if (!latestSnapshot && !loading) return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-900 to-slate-900 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-xl p-8 max-w-2xl border border-slate-500/30 text-center">
        <div className="text-6xl mb-4">📸</div>
        <h2 className="text-3xl font-bold text-white mb-4">Welcome to Persona Evolution</h2>
        <p className="text-slate-300 mb-6 text-lg leading-relaxed">Track how your communication style evolves over time.</p>
        <button onClick={handleCreateSnapshot} disabled={creatingSnapshot} className="bg-slate-600 hover:bg-slate-700 text-white px-8 py-4 rounded-lg font-bold text-lg disabled:opacity-50 transition-all">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-400">Track how your communication style evolves over time</p>
            <div className="mt-2 text-sm text-slate-500">{snapshots.length} snapshot{snapshots.length !== 1 ? 's' : ''} captured</div>
          </div>
          <div className="text-right">
            {canCreateSnapshot() ? (
              <button onClick={handleCreateSnapshot} disabled={creatingSnapshot} className="bg-slate-600 hover:bg-slate-700 text-white px-6 py-3 rounded-lg font-semibold disabled:opacity-50 transition-colors flex items-center gap-2">
              ...
              <div className="bg-slate-700/50 px-4 py-2 rounded-lg border border-slate-600">
                <div className="text-slate-400 text-sm">Next snapshot available:</div>
            ...
            <button onClick={() => setShowExplanation(!showExplanation)} className="mt-2 text-slate-400 hover:text-white text-sm underline block">What is this?</button>
          </div>
        </div>

        {showExplanation && (
          <div className="bg-slate-900/20 border border-slate-500/30 rounded-xl p-6 mb-6">
            <div className="flex justify-between items-start mb-4"><h3 className="text-xl font-bold text-white">How Persona Evolution Works</h3><button onClick={() => setShowExplanation(false)} className="text-slate-400 hover:text-white">✕</button></div>
        ...
        <div className="flex space-x-4 mb-6 border-b border-slate-700">
          {[
            ...
            <button key={tab.id} onClick={() => !tab.disabled && setSelectedTab(tab.id)} disabled={tab.disabled}
              className={`px-4 py-2 font-semibold transition-colors flex items-center gap-2 ${selectedTab === tab.id ? 'text-slate-400 border-b-2 border-slate-400' : tab.disabled ? 'text-slate-600 cursor-not-allowed' : 'text-slate-400 hover:text-white'}`}>
              {tab.label}
              {tab.badge && <span className="bg-slate-600 text-white text-xs px-2 py-0.5 rounded-full">{tab.badge}</span>}
            </button>
          ))}
        </div>
        ...
        {selectedTab === 'overview' && (
          <div className="space-y-6">
            <div className="bg-slate-800 rounded-xl p-6 border border-slate-500/30">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-2xl font-bold text-white mb-2">Latest Snapshot</h2>
                  <p className="text-slate-400">{new Date(latestSnapshot.timestamp).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</p>
                </div>
                <div className="text-right">
                  <div className="text-sm text-slate-400 mb-1">Drift Score</div>
                  ...
                  <div className="text-xs text-slate-500 mt-1">{!latestSnapshot.driftAnalysis?.overallDriftScore ? 'First snapshot' : latestSnapshot.driftAnalysis?.overallDriftScore > 50 ? 'Major change' : 'Minor change'}</div>
                </div>
              </div>
              <p className="text-slate-300 mb-6">{latestSnapshot.summary}</p>
              <div className="flex flex-wrap gap-3">
                <div className="bg-slate-900/50 px-4 py-2 rounded-full border border-slate-500/30"><span className="text-slate-400">Tone:</span> <span className="text-white font-semibold capitalize">{latestSnapshot.traits?.tone}</span></div>
                <div className="bg-slate-900/50 px-4 py-2 rounded-full border border-slate-500/30"><span className="text-slate-400">Style:</span> <span className="text-white font-semibold capitalize">{latestSnapshot.traits?.argumentativeStyle}</span></div>
                {perception?.trend && <div className="bg-slate-900/50 px-4 py-2 rounded-full border border-slate-500/30 flex items-center gap-2"><span className="text-slate-400">Perception:</span><TrendBadge trend={perception.trend} /></div>}
              </div>
            </div>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                {perception?.perceptionTraits && (
                  <div className="flex items-center gap-4 text-sm">
                    <span className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-slate-500 inline-block" /> You</span>
            ...
            {significantGaps.length > 0 && (
              <div className="bg-orange-900/20 border border-orange-500/30 rounded-xl p-6">
                <div className="flex justify-between items-center mb-4">
                  <div><h3 className="text-lg font-bold text-white">⚡ Perception Gap Detected</h3><p className="text-slate-400 text-sm mt-1">Significant difference between self and how others see you</p></div>
              ...
              <div className="bg-slate-800 rounded-xl p-6 border border-slate-500/30">
                ...
                <div className="space-y-3">
                  {latestSnapshot.keyChanges.map((change, idx) => {
                    ...
                    return (
                      <div key={idx} className={`p-4 rounded-lg border ${impact === 'high' ? 'bg-orange-900/20 border-orange-500/30' : impact === 'medium' ? 'bg-yellow-900/20 border-yellow-500/30' : 'bg-slate-900/20 border-slate-500/30'}`}>
                        <div className="flex justify-between items-start">
                          <div>
                            <div className="font-semibold text-white capitalize">{traitName}</div>
                            <div className="text-slate-400 text-sm mt-1">{change.description}</div>
                          </div>
                          <span className={`px-3 py-1 rounded-full text-xs font-semibold ${impact === 'high' ? 'bg-orange-500 text-white' : impact === 'medium' ? 'bg-yellow-500 text-black' : 'bg-slate-500 text-white'}`}>{impact}</span>
                        </div>
                      </div>
                    );
                  })}
            ...
            {latestSnapshot.topics?.primary?.length > 0 && (
              <div className="bg-slate-800 rounded-xl p-6 border border-slate-500/30">
                ...
                <div className="flex flex-wrap gap-2">
                  {latestSnapshot.topics.primary.map((topic, idx) => <span key={idx} className="bg-slate-600/30 text-slate-300 px-4 py-2 rounded-full border border-slate-500/50">{topic}</span>)}
                </div>
                {latestSnapshot.topics.emerging?.length > 0 && (
                  <div className="mt-4">
                    <div className="text-sm text-slate-400 mb-2">🌱 Emerging Topics</div>
        ...
        {selectedTab === 'perception' && (
          <div className="space-y-6">
            {perceptionLoading && <div className="text-center py-12 text-slate-400"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-slate-500 mx-auto mb-4" />Loading perception data...</div>}
            {!perceptionLoading && !perception?.perceptionTraits && (
              <div className="bg-slate-800 rounded-xl p-12 border border-slate-500/30 text-center">
                <div className="text-6xl mb-4">👥</div>
                <h3 className="text-2xl font-bold text-white mb-3">No Perception Data Yet</h3>
                <p className="text-slate-400 max-w-md mx-auto">Perception is built from anonymous Slicks others send you.</p>
              </div>
            )}
            {!perceptionLoading && perception?.perceptionTraits && (
              <>
                <div className="bg-slate-800 rounded-xl p-6 border border-orange-500/30">
                  <div className="flex justify-between items-start mb-4">
                    <div><h2 className="text-2xl font-bold text-white mb-1">How Others See You</h2><p className="text-slate-400 text-sm">Based on {perception.slickCount} anonymous Slick{perception.slickCount !== 1 ? 's' : ''} received{perception.updatedAt && ` · Updated ${new Date(perception.updatedAt).toLocaleDateString()}`}</p></div>
                    <TrendBadge trend={perception.trend} />
                  </div>
                  <p className="text-slate-300 mb-5 leading-relaxed">{perception.summary}</p>
                  <div className="flex flex-wrap gap-3">
                    <div className="bg-orange-900/30 px-4 py-2 rounded-full border border-orange-500/30"><span className="text-slate-400">Perceived Tone:</span> <span className="text-orange-300 font-semibold capitalize">{perception.perceptionTraits.tone}</span></div>
                    <div className="bg-orange-900/30 px-4 py-2 rounded-full border border-orange-500/30"><span className="text-slate-400">Perceived Style:</span> <span className="text-orange-300 font-semibold capitalize">{perception.perceptionTraits.argumentativeStyle}</span></div>
                  </div>
                </div>
                {latestSnapshot?.traits && (
                  <div className="bg-slate-800 rounded-xl p-6 border border-slate-500/30">
                    <div className="mb-6"><h3 className="text-xl font-bold text-white mb-1">Self vs Perception Gap</h3><p className="text-slate-400 text-sm"><span className="text-slate-400">Purple</span> = how you score yourself · <span className="text-orange-400">Orange</span> = how others perceive you</p></div>
                    ...
                    {latestSnapshot.traits.tone !== perception.perceptionTraits.tone && (
                      <div className="mt-6 bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4">
                        ...
                        <p className="text-slate-300 text-sm">You write in a <strong className="text-slate-300">{latestSnapshot.traits.tone}</strong> tone, but others perceive you as <strong className="text-orange-300">{perception.perceptionTraits.tone}</strong>.</p>
                      </div>
                    )}
                  </div>
                )}
                  ...
                  <div className="bg-slate-800 rounded-xl p-6 border border-slate-500/30">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {Object.entries(perception.toneBreakdown).map(([tone, count]) => (
                        <div key={tone} className="bg-slate-700/50 rounded-lg p-4 text-center"><div className="text-2xl font-bold text-white mb-1">{count}</div><div className="text-slate-400 text-sm capitalize">{tone}</div></div>
                      ))}
                    </div>
                  </div>
                )}
          ...
          <div className="space-y-6">
            {timeline.length < 2 ? (
              <div className="bg-slate-800 rounded-xl p-12 border border-slate-500/30 text-center">
                ...
                <p className="text-slate-400 mb-4">Create at least 2 snapshots to see your evolution timeline</p><p className="text-sm text-slate-500">Current snapshots: {timeline.length}</p>
              </div>
            ) : (
              <>
                <div className="bg-slate-800 rounded-xl p-6 border border-slate-500/30">
                  ...
                  <div className="bg-slate-800 rounded-xl p-6 border border-slate-500/30">
                    ...
                    <div className="space-y-4">
                      {significantDrifts.map((drift) => (
                        <div key={drift.id} className="bg-slate-700/50 p-4 rounded-lg border border-slate-600">
                          <div className="flex justify-between items-start mb-2"><div className="text-sm text-slate-400">{new Date(drift.timestamp).toLocaleDateString()}</div><div className="text-orange-400 font-bold">Drift: {drift.driftScore}</div></div>
                          <p className="text-slate-300">{drift.summary}</p>
                          {drift.significantChanges?.length > 0 && (
                            <div className="mt-3 space-y-2">
                              {drift.significantChanges.map((change, cidx) => <div key={cidx} className="text-sm text-slate-400 pl-4 border-l-2 border-slate-500/50">• {change?.description || ''}</div>)}
                            </div>
                          )}
                        </div>
                      ))}
          ...
          <div className="space-y-6">
            {(!evolutionStats || evolutionStats.totalSnapshots < 2) ? (
              <div className="bg-slate-800 rounded-xl p-12 border border-slate-500/30 text-center">
                ...
                <p className="text-slate-400 mb-4">Trait evolution compares snapshots over time, so it needs at least 2. Keep debating and a new one is captured automatically.</p><p className="text-sm text-slate-500">Current snapshots: {evolutionStats?.totalSnapshots || snapshots.length || 0}</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[{ label: 'Total Snapshots', value: evolutionStats.totalSnapshots }, { label: 'Duration', value: `${evolutionStats.durationDays} days` }, { label: 'First Snapshot', value: new Date(evolutionStats.firstSnapshot).toLocaleDateString() }].map(stat => (
                    <div key={stat.label} className="bg-slate-800 rounded-xl p-6 border border-slate-500/30"><div className="text-slate-400 text-sm mb-1">{stat.label}</div><div className="text-3xl font-bold text-white">{stat.value}</div></div>
                  ))}
                </div>
                      ...
                      {Object.entries(evolutionStats.traitEvolution).map(([trait, data]) => (
                        <div key={trait} className="bg-slate-700/50 p-4 rounded-lg">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <div className={`font-bold ${data.change > 0 ? 'text-green-400' : data.change < 0 ? 'text-red-400' : 'text-slate-400'}`}>{data.change > 0 ? '+' : ''}{data.change}</div>
                          </div>
